# 🛠️ TAREA ACTUAL
**ID:** #035 (reapertura) | **Estado:** 🟡 EN CURSO — FASE 1 ✅, #038 ✅, FASE 2 ✅, FASE 3 ✅, FASE 4 ✅, FASE 5 ✅ (`9897df3`), FASE 6 ✅ (`e475a2c`), FASES 7-8 ✅ (`2d361a0`); tarea implementada completa, pendiente validación final E2E + cierre | **Fecha:** 2026-08-27
...
- **Lo último que funcionó:**
  ...
  - **FASE 3 #035 COMPLETADA ✅** (cliente): extracción de modales inline a `client/src/components/common/Dashboard/ReportActivityModals.jsx` como presentacionales reutilizables `{open,data,onClose}`. `BitacoraDetailModal` (con `renderFooter` y `showAuthor` opcionales) y `WorkPhotoModal` (con `renderActions`, `showEmployee`, info de revisión). Refactorizadas las 4 páginas: `hrbitacoraspage.jsx`, `HRWorkPhotosPage.jsx` (actions review/delete), `bitacoraspage.jsx` (empleado, footer Editar + `showAuthor=false`), `EmployeeWorkPhotosPage.jsx` (`showEmployee=false`). −325 líneas de duplicación. **Verificación en vivo OK** (commit `3571af8` + fix `3667a1d`).
  - **VERIFICACIÓN EN VIVO FASE 3 ✅** (2026-08-28): servicios levantados §14.3. DB poblada verif. (ORGS1/HR1/EMPS24/5 snapshots). Smoke tests: health ok, login HR+Emp ok, rutas SPA de las 4 páginas HTTP 200, endpoints `bitacora/all`, `workphoto/all`, `my-bitacoras`, `my-photos` OK con datos que alimentan modales (bitácora refId del reporte tiene content+images+employee; foto tiene photourl+workdate+employee). **Hallazgo corregido:** bitácoras de empleado traen `employee` poblado → habría activado header con autor en modal del empleado (regresión vs original). Fix `showAuthor` prop + `showAuthor={false}` en `bitacoraspage.jsx`. Client build 0 errores + 61/61 server tests. Servicios detenidos; Mongo Up con datos. [CTO ordenó detener Mongo — hecho.]
  - **FASE 4 #035 COMPLETADA ✅** (cliente): interactividad en `ReportComponents.jsx` — componentes listos para recibir callbacks opcionales SIN romper consumidores existentes: `ActivityRow` acepta `onClick` (cursor-pointer, hover, role=button, accesible Enter/Espacio); `TotalChip` acepta `onClick` (hover con elevación); `ReportTotalsBar` acepta `onChipClick(key)` con keys `bitacoras|tasks|photos|checkIns|horas` (horas no navegable); `EmployeeActivityGroup` acepta `onActivityClick(activity)` y lo propaga a ActivityRow. **NO rompe regresiones:** los consumidores actuales (HRReportPage, EmployeeHomePage, ReportCompactCard) no pasan handlers → siguen como display. Client build 0 errores + 61/61 server tests.
  - **FASE 5 #035 COMPLETADA ✅** (commit `9897df3`, cliente): integración de interactividad en `HRReportPage.jsx` — `useNavigate` + navegación por query params (`?from=&to=`) al clickear chips del `ReportTotalsBar` (diario: `dailyWindow`, semanal: `weeklyWindow`; `tasks`→`/HR/dashboard/schedules?day=`); `onActivityClick` abre modales `BitacoraDetailModal` (muestra `title`+`description` del reporte, sin content completo) o `WorkPhotoModal` (datos directos `meta.photourl`+`title`+`date`, sin fetch) vía estado `activeActivity`. Propágado `onActivityClick` también a las activities del `SnapshotDetailModal` (histórico). `dailyWindow`/`weeklyWindow` confirmados con `.start`/`.end` en `server/utils/reportWindow.util.js` + `Report.controller.js`. Client build 0 errores + 61/61 server tests.
  - **FASE 6 #035 COMPLETADA ✅** (commit `e475a2c`, cliente): filtros **URL-driven** en las 4 páginas destino + alineación del contrato. **Corrección clave:** el query param de navegación se renombró de `from`/`to` → **`startDate`/`endDate`** para coincidir exactamente con el contrato server-side de la Fase 2 (Bitacora.controller GET_ALL/GET_MY + WorkPhoto.controller getAll/my). Thunks ampliados para aceptar params (sin romper llamadas existentes): `HandleGetAllBitacoras` (ya lo hacía), `HandleGetMyBitacoras`, `HandleHRWorkPhoto` (`GetAll`), `HandleEmployeeDashboard` (`MyPhotos`). Páginas destino leen `?startDate=&endDate=` y pasan al fetch server-side: `hrbitacoraspage.jsx` (+ puebla `dateRange` UI a `YYYY-MM-DD`), `HRWorkPhotosPage.jsx` (GetAll + `filters.date` inicial), `bitacoraspage.jsx`, `EmployeeWorkPhotosPage.jsx`. Hay 3 thunks con `[dispatch, searchParams]` para re-fetch al cambiar rango en misma ruta. Client build 0 errores + 61/61 server tests.
  - **FASES 7-8 #035 COMPLETADAS ✅** (commit `2d361a0`, cliente): interactividad **lado empleado** — `EmployeeHomePage.jsx`: `useNavigate` + `navigateChip` hacia rutas empleado (`/auth/employee/employee-dashboard/{bitacoras,photos,schedule}`) con `?startDate=&endDate=` usando `dailyWindow||weeklyWindow`; `onActivityClick` abre `BitacoraDetailModal`/`WorkPhotoModal` vía `activeActivity`. `EmployeeReportsPage.jsx`: modales de detalle en `MyWeekDetailModal` (histórico propio) con `onActivityClick` propagado. Ambas páginas destino empleado ya preparadas en Fase 6. Client build 0 errores + 61/61 server tests.
- **Dónde se rompió/detuvo:** N/A. Fases 7-8 commit `2d361a0` pusheado. **Implementación de la tarea #035 COMPLETA** (R1+R2+R3). Pendiente: validación E2E en vivo (servicios) + STOP aprobación CTO + cierre.
- **Siguiente acción inmediata:** (1) Validación E2E en vivo (§14 local-deploy): levantar servicios, login HR/Empleado, verificar chips→navegación con filtros y modales de detalle en 4 vistas (HR diario/semanal/snapshot, Empleado home/reports). (2) STOP aprobación CTO. (3) Confirmación tarea completada → actualizar bitácoras + auto-mantenimiento + PR hacia dev.
